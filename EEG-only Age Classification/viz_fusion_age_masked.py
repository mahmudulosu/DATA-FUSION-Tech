# viz_fusion_age_masked.py
# EEG+MRI age (young/old) visualization with *brain-masked* Grad-CAM.
# - Uses same TEST split (subject-level) as training (no leakage)
# - CAM is normalized *only inside brain* to avoid background artifacts
# - Per-subject panels + group average NIfTI/PNGs
# - Fused TEST embeddings -> t-SNE + LDA sanity plots
...
def build_age_label_map(csv_path: str):
    ...
    lab = {"young":0,"old":1}
    return {row["participant_id"]: lab[age_group[row["age"]]] for _,row in df.iterrows()}
...
class MRIDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for _ in range(self.k_crops):
            ...
            v=torch.from_numpy(v).unsqueeze(0).repeat(3,1,1,1) # [3,128^3]
            crops.append(v)
        crops=torch.stack(crops,0)  # [K,3,128^3]
...
def make_brain_mask_160(vol160, thr=0.10):
    """Create a simple brain mask in 160^3 space from normalized volume."""
...
def normalize_cam_inside_mask(cam128, brain_mask_160):
    """Normalize CAM using only brain voxels; zeros outside brain."""
    cam160 = put_128_into_160(cam128)
    m = brain_mask_160.astype(bool)
    if m.sum() == 0:
        return cam160  # fallback
    v = cam160[m]
...
def gradcam_3d_for_crop(mri_enc, head, crop_tensor, e_feat, class_idx=None, cam_layer="layer3"):
    ...
    cam_layer: "layer3" (sharper) or "layer4" (coarser)
    """
    mri_enc.eval(); head.eval()
    e_fixed = e_feat.detach()
    target = getattr(mri_enc.backbone, cam_layer)
    hook = CAMHook(target)

    with torch.enable_grad():
        crop_tensor = crop_tensor.requires_grad_(False)
...
def cams_group(eeg_enc, mri_enc, head, loader, k_crops=3, cam_layer="layer3"):
    eeg_enc.eval(); mri_enc.eval(); head.eval()
    ...
    for xe,xm,y,ids,vol in tqdm(loader, desc="MRI Grad-CAM (masked)"):
        ...
        att_w=att.squeeze(0).cpu().numpy(); att_w=att_w/(att_w.sum()+1e-8)
...
def save_group_cam_nifti(avg_maps, out_dir):
    ...
    for c, cam in avg_maps.items():
        ...
        overlay_panels_masked(np.ones_like(big), cam,   # blank background for group panel
...
if __name__=="__main__":
    ...
    # 2) MRI Grad-CAM (brain-masked), choose cam_layer="layer3" for sharper maps
    print("Computing brain-masked MRI Grad-CAM (group averages)...")
    cam_avg, n_cam = cams_group(eeg_enc, mri_enc, head, te_loader, k_crops=K_MRI_CROPS, cam_layer="layer3")
    print(f"Subjects per class (test): young={n_cam[0]}, old={n_cam[1]}")
